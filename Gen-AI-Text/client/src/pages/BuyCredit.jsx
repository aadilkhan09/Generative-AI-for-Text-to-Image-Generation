      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, index)=> (
         ...
         duration-500'>
            ...
            <p className='mt-6'><span className='text-3xl'>₹{item.id}</span></p>
            <button onClick={()=>paymentRazorpay(item.id)} className='w-full bg-blue-600 text-white mt-8 text-sm rounded-full py-2.5 min-w-52'>{user ? 'Purchase' : 'Get Started'}Purchase</button>
          </div>
        ))}
      </div>
    </motion.div>
